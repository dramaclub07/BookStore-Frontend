...
// }
const BASE_URL = "https://bookstore-backend-p7e1.onrender.com/api/v1/";
// Note: Ensure REDIS_URL is set in Render environment (e.g., redis://<user>:<pass>@<host>:<port>)
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    await Promise.all([loadUserProfile(), fetchAddresses(), loadCartSummary()]);
    setupHeaderEventListeners();
  } catch (error) {
    console.error("Initialization error:", error.message);
    alert("Failed to load profile or addresses. Please try again.");
  }
});
...
async function fetchAddresses() {
  try {
    ...
    document.getElementById("address-list").innerHTML =
      "<p>No addresses found or error loading addresses.</p>";
    return [];
  }
}
...
function renderAddresses(addresses) {
  ...
  addresses.forEach((address, index) => {
    const addressId = address.id;
    const addressType = address.address_type || "unknown";
    const isEditing = isEditingAddress[addressId] || false;

    const addressElement = document.createElement("div");
    addressElement.className = "work-address";
    addressElement.id = `address-${addressId}`;

    addressElement.innerHTML = `
      <div class="work-header">
        <div class="work-title">${index + 1}. ${addressType.toUpperCase()}</div>
        <div class="action-buttons">
          <a href="#" class="address-edit-btn" onclick="toggleEdit('address-${addressId}'); return false;">${
      isEditing ? "Cancel" : "Edit"
    }</a>
          <a href="#" class="address-select-btn" onclick="selectAddress(${addressId}); return false;">Select</a>
          <a href="#" class="address-delete-btn" onclick="deleteAddress(${addressId}); return false;">Delete</a>
        </div>
      </div>
      <div class="address-details">
        ${
          ...
          <div class="form-group">
            <label for="street-${addressId}">Address</label>
            <textarea id="street-${addressId}" ${isEditing ? "" : "disabled"}>${
                address.street || ""
              }</textarea>
          </div>
          <div class="address-row">
            <div class="address-col">
              <div class="form-group">
                <label for="city-${addressId}">City/Town</label>
                <input type="text" id="city-${addressId}" value="${
                address.city || ""
              }" ${isEditing ? "" : "disabled"}>
              </div>
            </div>
            <div class="address-col">
              <div class="form-group">
                <label for="state-${addressId}">State</label>
                <input type="text" id="state-${addressId}" value="${
                address.state || ""
              }" ${isEditing ? "" : "disabled"}>
              </div>
            </div>
          ...
          <div class="address-row">
            <div class="address-col">
              <div class="form-group">
                <label for="zip-code-${addressId}">Zip Code</label>
                <input type="text" id="zip-code-${addressId}" value="${
                address.zip_code || ""
            ...
            <div class="address-col">
              <div class="form-group">
                <label for="country-${addressId}">Country</label>
                <input type="text" id="country-${addressId}" value="${
                address.country || ""
              }" ${isEditing ? "" : "disabled"} required>
              </div>
            </div>
          ...
          <div class="form-group radio-group">
            <label>Type</label>
            <label><input type="radio" name="address-type-${addressId}" value="home" ${
                addressType === "home" ? "checked" : ""
              } ${isEditing ? "" : "disabled"}> Home</label>
            <label><input type="radio" name="address-type-${addressId}" value="work" ${
                addressType === "work" ? "checked" : ""
              } ${isEditing ? "" : "disabled"}> Work</label>
            <label><input type="radio" name="address-type-${addressId}" value="other" ${
                addressType === "other" ? "checked" : ""
              } ${isEditing ? "" : "disabled"}> Other</label>
          </div>
          ${
            isEditing
              ? `<button id="save-address-${addressId}" onclick="saveAddress(${addressId}); return false;">Save</button>`
              : ""
          }
        `
            : `
...
function toggleAddressForm() {
  ...
  if (addressForm) {
    addressForm.classList.toggle("show", isAddressFormVisible);
    if (!isAddressFormVisible) {
      document.getElementById("new-street").value = "";
      document.getElementById("new-city").value = "";
      document.getElementById("new-state").value = "";
      ...
      document.querySelector(
        'input[name="new-address_type"][value="home"]'
      ).checked = true;
    }
  }
}
...
async function saveNewAddress() {
  ...
  const addressType = document.querySelector(
    'input[name="new-address_type"]:checked'
  ).value;
  const newAddress = {
  ...
  if (
    ...
    alert("Street, city, state, and country are required.");
    return;
  }
  ...
  try {
    ...
    console.error("New address save error:", error.message);
    alert(error.message);
  }
}

async function saveAddress(addressId) {
  const zipCode = document.getElementById(`zip-code-${addressId}`).value.trim();
  if (!zipCode) {
    alert("Zip Code is required.");
    return;
  }
  ...
  const updatedAddress = {
    street: document.getElementById(`street-${addressId}`).value.trim(),
    city: document.getElementById(`city-${addressId}`).value.trim(),
    state: document.getElementById(`state-${addressId}`).value.trim(),
    zip_code: zipCode,
    country: document.getElementById(`country-${addressId}`).value.trim(),
    address_type: document.querySelector(
      `input[name="address-type-${addressId}"]:checked`
    ).value,
  };
...
async function selectAddress(addressId) {
  try {
    ...
    console.error("Address selection error:", error.message);
    alert(error.message);
  }
}
